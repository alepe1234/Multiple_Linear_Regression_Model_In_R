...
We pick up the movie "La la land" and fill the data in a data frame.
...
The model is built on a small sample of around 650 movie entries and the 
prediction it gave for our test movie was not so good, with an error of 13%.

We have chosen a difficult test movies as there are not so many movies with 
genre = 'Musial' in the dataset. Actually for many genres there are very few 
movies, only dramas are well represented (about 50% of the dataset). 
